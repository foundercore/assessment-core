...
public class TestAssignmentServiceImpl implements TestAssignmentService {
	...
	public String assignTest(AssignmentRequestDto dto) {
		...
		if (!isTestAssignable(dto.getTestId())) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Test not published. Id - %s", dto.getTestId()));
		}
	...
	private Assignment getAssignmentEntity(String assignmentId) {
		...
		return assignmentRepository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
	...
	public void deleteAssignment(String assignmentId) {
		...
		if (!isSubmissionExists(assignment.getId().getAssignmentId())) {
			...
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					"Can not delete assignment. Student submission exists.");
		}
	}
	...
	public List<AssignmentResponseDto> listStudentAssignmentsByUsername(String username) {
		User user = userService.getUser(username).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
	...
	public boolean isValidAssignmentPasscode(String assignmentId, String username, String passcode) {
		Assignment assignment = getAssignmentEntity(assignmentId);
		String email = userService.getUser(username)
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
						String.format("Student not found. Student Id - %s", username)))
				.getEmail();
		...
		throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
				String.format("Assignment %s not assigned to student %s", assignmentId, username));
	}
	...
	private Submission getSubmissionEntity(String assignmentId, String studentId, boolean suppressException) {
		...
		if (submission == null && !suppressException) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format(
					"Student submission details not found. Student - %s, Assignment - %s", studentId, assignmentId));
		}
		return submission;
	}
	...
	private Submission initStudentSubmission(String assignmentId, String studentId) {
		...
		return submissionRepository.findById(id)
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
						String.format("Unable to initialize student submission. Assignment - %s, Student - %s",
	...
	public void saveStudentAnswer(String assignmentId, SaveAnswerRequestDto dto) {
		...
		if (submission.isSubmitted()) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Can't update test submission state. Test %s already submitted by student %s",
							assignmentId, studentId));
		}
	...
	public synchronized void initiateAssignmentEvaluation(String assignmentId) {
		...
		for (Submission submission : submissions) {
			if (!submission.isSubmitted()) {
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
						String.format("Test evaluation can not start. Submission not yet over for student %s",
		...
		/*
		 * iterate over each section, each question & update correct/incorrect, skipped,
		 * marks
		 */
		for (Submission submission : submissions) {
			if (!EvaluationState.PENDING.value().equalsIgnoreCase(submission.getEvaluation())) {
				log.warn(
						"[Skipped] Student assignment evaluation already initiated/completed. Assignment - {}, Student - {}, Evaluation State - {}",
						assignmentId, submission.getStudentId(), submission.getEvaluation());
				continue;
			}
	...
	@Override
	public synchronized void initiateStudentAssignmentEvaluation(String assignmentId, String studentId,
		...
		if (submission == null) {
			...
			mongoTemplate.save(assignment);
			log.warn(
					"Evaluation Completed. Student absent or not yet participated in assignment. Assignment - {}, Student - {}",
					assignmentId, studentId);
			return;
		}

		/*
		 * remove if student found in absent list. This will come if evaluation starts
		 * before student participates in exam
		 */
		...
		if (!EvaluationState.PENDING.value().equalsIgnoreCase(submission.getEvaluation()) && !forceEvaluation) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format(
					"Student assignment evaluation already initiated/completed. Assignment - %s, Student - %s, Evaluation State - %s",
					assignmentId, submission.getStudentId(), submission.getEvaluation()));
		}
	...
	public SubmissionResponseDto viewStudentAssignmentResult(String assignmentId, String username) {
		...
		Submission submission = getSubmissionEntity(assignmentId, username, false);
		if (EvaluationState.PENDING.value().equalsIgnoreCase(submission.getEvaluation())) {
			log.warn("Student evaluation started. Assignment - {}, Student - {}", submission.getAssignmentId(),
					submission.getStudentId());
	...
	public void triggerScheduledStudentSubmissionEvaluation(String tenantId) {
		...
		criteria.andOperator(Criteria.where("submitted").is(true),
				Criteria.where("evaluation").ne(EvaluationState.COMPLETED.value()));
		query.addCriteria(criteria);
		List<Submission> submissions = mongoTemplate.find(query, Submission.class);
		for (Submission submission : submissions) {
			try {
	...
	private synchronized void initStudentAssignmentEvaluation(Submission submission, Map<String, Question> questionMap,
			QuestionPaperResponseDto testConfig) {
		...
		log.info("Evaluation completed for student - {}, assignment - {}", submission.getStudentId(),
				submission.getAssignmentId());
	}

	private void sectionEvaluation(Submission submission, QuestionPaperResponseDto.PaperSectionResponseDto sectionDto,
			Map<String, List<Submission.Answer>> missing, Map<String, Question> questionMap) {
				...
				for (QuestionPaperResponseDto.TestQuestionResponseDto questionDto : sectionDto.getQuestions()) {
					if (section.getAnswers().containsKey(questionDto.getId())) {
						...
						if ((answer.getOptions() == null || answer.getOptions().isEmpty())
								&& StringUtils.isEmpty(answer.getAnswerText())) {
