...
public class TestConfigServiceImpl implements TestConfigService {
    ...
    public QuestionPaper getEntity(String paperId) {
        ...
        id.setTenantId(AuthUtils.getCurrentTenantId());
        id.setQuestionPaperId(paperId);
        return testConfigRepository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question paper %s does not exist", paperId)));
    }
    ...
    private void validateQuestionPaperDto(QuestionPaperRequestDto dto){
        ...
        if (dto.getTotalDurationInMinutes() < 0){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("totalDurationInMinutes %s can't be negative", dto.getTotalDurationInMinutes()));
        }
        ...
        if (dto.getSections() != null) {
            dto.getSections().forEach(paperSectionRequestDto -> {
                if (names.contains(paperSectionRequestDto.getName())) {
                    throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Section with name %s already exist", paperSectionRequestDto.getName()));
                }
                if (paperSectionRequestDto.getQuestions() != null) {
                    paperSectionRequestDto.getQuestions().forEach(questionDto -> {
                        ...
                        if (questions.contains(questionDto.getId())) {
                            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question %s occurs multiple times in question paper", questionDto.getId()));
                        }
    ...
    private void validateTestQuestion(QuestionPaperRequestDto.TestQuestionRequestDto questionDto){
        if (questionService.exists(questionDto.getId())) {
            if (questionDto.getPositiveMark() < 0){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Invalid positiveMark for question %s ", questionDto.getId()));
            }
            if (questionDto.getNegativeMark() < 0){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Invalid negativeMark for question %s ", questionDto.getId()));
            }
            if (questionDto.getSkipMark() < 0){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Invalid skipMark for question %s ", questionDto.getId()));
            }
        }else {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question with Id %s does not exist", questionDto.getId()));
        }
    }
...
//        }
    ...
    @Override
    public String addQuestionPaperSection(String paperId, QuestionPaperRequestDto.PaperSectionRequestDto paperSectionRequestDto) {
        QuestionPaperId id = new QuestionPaperId();
        id.setTenantId(AuthUtils.getCurrentTenantId());
        id.setQuestionPaperId(paperId);
        QuestionPaper questionPaper = testConfigRepository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question paper %s does not exist", paperId)));
        ...
        if (paperSectionRequestDto.getQuestions() != null) {
            for (QuestionPaperRequestDto.TestQuestionRequestDto dto : paperSectionRequestDto.getQuestions()) {
                if (questions.contains(dto.getId())) {
                    throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question %s already exist in other section", dto.getId()));
                }
            }
        }
    ...
    @Override
    public void updateQuestionPaperSectionMetadata(String paperId, String sectionId, QuestionPaperRequestDto.PaperSectionRequestDto dto) {
        QuestionPaperId id = new QuestionPaperId();
        id.setTenantId(AuthUtils.getCurrentTenantId());
        id.setQuestionPaperId(paperId);
        QuestionPaper questionPaper = testConfigRepository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question paper %s does not exist", paperId)));
    ...
    @Override
    public void addNewQuestionsInQuestionPaperSection(String paperId, String sectionId, List<QuestionPaperRequestDto.TestQuestionRequestDto> questionDtos) {
        QuestionPaperId id = new QuestionPaperId();
        id.setTenantId(AuthUtils.getCurrentTenantId());
        id.setQuestionPaperId(paperId);
        QuestionPaper questionPaper = testConfigRepository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question paper %s does not exist", paperId)));
        ...
        for (QuestionPaperRequestDto.TestQuestionRequestDto dto: questionDtos){
            if (questions.contains(dto.getId())){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question %s already exist in question paper", dto.getId()));
            }
        }
    ...
    public void removeQuestionsFromQuestionPaperSection(String paperId, String sectionId, List<String> questions) {
        ...
        for (String question: questions){
            if (section.getQuestions() == null || !section.getQuestions().containsKey(question)){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question with Id %s does not exist in paper %s, section %s", question, paperId, sectionId));
            }
        }
    ...
    @Override
    public void updateQuestionsInQuestionPaperSection(String paperId, String sectionId, List<QuestionPaperRequestDto.TestQuestionRequestDto> questionDtos) {
        QuestionPaperId id = new QuestionPaperId();
        id.setTenantId(AuthUtils.getCurrentTenantId());
        id.setQuestionPaperId(paperId);
        QuestionPaper questionPaper = testConfigRepository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question paper %s does not exist", paperId)));
        ...
        for (QuestionPaperRequestDto.TestQuestionRequestDto dto: questionDtos){
            if (section.getQuestions() == null || !section.getQuestions().containsKey(dto.getId())){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question with Id %s does not exist in paper %s, section %s", dto.getId(), paperId, sectionId));
            }
        }
    ...
    public void initiateQuestionPaperVerification(String paperId) {
        ...
        if (!QuestionPaperStatus.DRAFT.value().equalsIgnoreCase(questionPaper.getStatus())){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Can't init verification for question paper %s. Status not draft.", paperId));
        }
    ...
    private void validateTestForStateMigration(QuestionPaper questionPaper) {
        if (questionPaper.getSections() == null || questionPaper.getSections().isEmpty()){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question paper sections missing. Paper id - %s", questionPaper.getId().getQuestionPaperId()));
        }
        for (QuestionPaper.PaperSection section: questionPaper.getSections().values()){
            if (StringUtils.isEmpty(questionPaper.getName())){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Section name missing. Paper id - %s, Section id - %s", questionPaper.getId().getQuestionPaperId(), section.getId()));
            }
            if (questionPaper.getSubsections(section.getId()).isEmpty() && (section.getQuestions() == null || section.getQuestions().isEmpty())){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Questions missing in section. Paper id - %s, Section - %s", questionPaper.getId().getQuestionPaperId(), section.getId()));
            }
        }
        ...
        questionPaper.getSections().forEach((k1, section) -> {
            ...
            if (names.contains(section.getName().toLowerCase())) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Duplicate Section name %s ", section.getName()));
            }
            if (section.getQuestions() != null) {
                section.getQuestions().forEach((k2, question) -> {
                    ...
                    if (questionService.exists(question.getId())) {
                        ...
                        if (question.getPositiveMark() < 0) {
                            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Invalid positiveMark for question %s ", question.getId()));
                        }
                        if (question.getNegativeMark() < 0) {
                            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Invalid negativeMark for question %s ", question.getId()));
                        }
                        if (question.getSkipMark() < 0) {
                            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Invalid skipMark for question %s ", question.getId()));
                        }
                    } else {
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question with Id %s does not exist", question.getId()));
                    }
                    /* duplicate question */
                    if (questions.contains(question.getId())) {
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question %s occurs multiple times in question paper", question.getId()));
                    }
        ...
        if (questions.isEmpty()){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Questions missing in paper. Paper id - %s", questionPaper.getId().getQuestionPaperId()));
        }
    }
    ...
    public void verifyQuestionPaper(String paperId) {
        ...
        if (!QuestionPaperStatus.PENDING_VERIFICATION.value().equalsIgnoreCase(questionPaper.getStatus())){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Can't verify question paper %s. Verification not initiated.", paperId));
        }
        Set<ConstraintViolation<QuestionPaper>> violations = validator.validate(questionPaper);
        if (!violations.isEmpty()) {
            throw new ConstraintViolationException(violations);
        ...
        QuestionPaper.PaperMigration activity = questionPaper.getMigration().get(questionPaper.getMigration().size() -1);
        if (activity.getActivityBy().equalsIgnoreCase(AuthUtils.getCurrentQualifiedUsername()) && !isAdminUser()){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Self verification not allowed. Paper id - %s", paperId));
        }
    ...
    public void rejectQuestionPaperVerification(String paperId, String rejectionReason) {
        ...
        if (!QuestionPaperStatus.PENDING_VERIFICATION.value().equalsIgnoreCase(questionPaper.getStatus())){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Can't process verification request for question paper %s. Verification not initiated.", paperId));
        }
    ...
    public void archiveQuestionPaper(String paperId) {
        ...
        if (!QuestionPaperStatus.PUBLISHED.value().equalsIgnoreCase(questionPaper.getStatus())){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Can't archive question paper. Status not published."));
        }
    ...
    public Question getQuestionPaperLinkedQuestion(String paperId, String questionId) {
        ...
        if (questions.isEmpty()){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Question %s not found in repository. Paper %s ", questionId, paperId));
        }
    ...
    public void publishQuestionPaper(String paperId) {
        ...
        if (!QuestionPaperStatus.VERIFIED.value().equalsIgnoreCase(questionPaper.getStatus())){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Can't publish question paper %s. Verification not done.", paperId));
        }
        Set<ConstraintViolation<QuestionPaper>> violations = validator.validate(questionPaper);
        if (!violations.isEmpty()) {
            throw new ConstraintViolationException(violations);
    ...
    public QuestionPaperPaginatedResponse searchQuestionPapers(SearchQuestionPaperDto filter) {
        ...
        if (StringUtils.isNotEmpty(filter.getUpdateEndTime())){
            if (rangeCriteria != null){
                ...
                query.addCriteria(Criteria.where("lastUpdatedOn")
...
//            response.setPaginatedRowId(dtos.get(dtos.size()-1).getQuestionPaperId());
    ...
    private void updateQuestionRepository(QuestionPaper questionPaper){
        ...
        if (questionPaper.getSections() != null) {
            ...
            questionService.markQuestionUsage(questionPaper.getId().getQuestionPaperId(), questionPaper.getStatus(), questions);
        }
    }
}
