...
import com.assessment.question.dto.QuestionPaginatedResponse.UsedInTestDetails;
...
public class QuestionServiceImpl implements QuestionService {
	...
	public String generatePassageId(String passageContent, boolean suppressException) {
		if (StringUtils.isEmpty(passageContent)) {
			if (!suppressException)
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Passage content is empty");
			return "";
		}
	...
	public void updatePassage(String passageId, Passage passage) {
		...
		Passage currentPassage = passageRepository.findById(id)
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
						String.format("Passage with id %s not found", passageId)));
	...
	public void linkQuestionsToPassage(String passageId, List<String> questionIds) {
		...
		passageRepository.findById(passageId1).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
				String.format("Passage with id %s not found", passageId)));
		...
		for (String questionId : questionIds) {
			...
			if (passageId.equalsIgnoreCase(question.getPassageId())) {
				...
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
						String.format("Question with id %s not linked to passage", questionId));
			}
		}
		questionRepository.saveAll(questions);
	}
	...
	public void initBulkCreationQuestion(String fileName) throws IOException, CsvValidationException {
		...
		while (decoder.hasNext()) {
			...
			/*
			 * migration fields added to do migration without missing anything from existing
			 * source. New files won't have these fields so it won't impact
			 */
			...
			if (exists(id.getQuestionId())) {
				log.info("Question {} already exist in system. File - {}, Position - {}", id.getQuestionId(), fileName,
						rowCount);
				continue;
			}
			...
			for (Map.Entry<String, Object> entry : record.entrySet()) {
				...
				default:
					if (key.startsWith("option") && key.contains("_") && key.split("_").length == 2) {
						...
						String message = String.format("Unknown key %s found in question bulk upload file %s", key,
								fileName);
		...
		if (error.toString().trim().isEmpty()) {
			log.info("File - {}, Saved - {}, Duplicates - {}", fileName, unique.size(),
	...
	public void updateQuestion(Question question) {
		Question currentQuestion = questionRepository.findById(question.getId())
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
						String.format("Question with id %s not found", question.getId().getQuestionId())));
	...
	private List<QuestionResponseDto> updateReferences(List<Question> questions, SearchQuestion searchFilter) {

		List<QuestionResponseDto> questionResponseData = new ArrayList<>();
		for (Question question : questions) {

			/* create response DTO */
			QuestionResponseDto questionResponseDto = question.getResponseDto();
			/* update passage content */
			updateQuestionPassageReference(questionResponseDto);
			/* update used test details */
			updateUsedInTestReference(searchFilter.isIncludeUsedTestDetails(), questionResponseDto);
			questionResponseData.add(questionResponseDto);
		}
		return questionResponseData;
	}

	private void updateUsedInTestReference(boolean isIncludeUsedTestDetails, QuestionResponseDto questionResponseDto) {
		/* update used test details */
		if (isIncludeUsedTestDetails && questionResponseDto.getUsedInPapers() != null
				&& !questionResponseDto.getUsedInPapers().isEmpty()) {
			questionResponseDto.setUsedInTestDetails(new ArrayList<>());
			for (String testId : questionResponseDto.getUsedInPapers().keySet()) {
				QuestionPaperId id = new QuestionPaperId();
				id.setTenantId(AuthUtils.getCurrentTenantId());
				id.setQuestionPaperId(testId);
				QuestionPaper questionPaperUsedIn = testConfigRepository.findById(id).orElse(null);
				/* if question found add the details in the response DTO. */
				if (questionPaperUsedIn != null) {
					questionResponseDto.getUsedInTestDetails().add(new UsedInTestDetails(testId,
							questionPaperUsedIn.getStatus(), questionPaperUsedIn.getName()));
				}
	...
	private void updateQuestionPassageReference(QuestionResponseDto question) {
		/* update passage content */
	...
	public void validateQuestion(Question question) {
		...
		if (!QuestionType.getQuestionTypes().contains(question.getType())) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Question Type %s not valid", question.getType()));
		}
		...
		if (QuestionType.PASSAGE.value().equalsIgnoreCase(question.getType())) {
			if (StringUtils.isEmpty(question.getPassageId())) {
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
						String.format("Passage content not supplied for question type %s", question.getType()));
			}
			...
			passageRepository.findById(passageId).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Passage with id %s not found", question.getPassageId())));
		}
		...
		if ((QuestionType.MCQ.value().equalsIgnoreCase(question.getType())
			...
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Options not supplied for question type %s", question.getType()));
		}
		...
		if (QuestionType.MCQ.value().equalsIgnoreCase(question.getType())
			...
			for (Question.QuestionOption option : question.getOptions()) {
				if (StringUtils.isEmpty(option.getKey()) || StringUtils.isEmpty(option.getValue())) {
					throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
							String.format("Invalid option key/value for question type %s", question.getType()));
				}
			}
		}
		...
		if (QuestionType.MCQ.value().equalsIgnoreCase(question.getType())
			...
			for (Question.QuestionOption option : question.getOptions()) {
				if (keys.contains(option.getKey())) {
					throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
							String.format("Options keys %s must be unique", option.getKey()));
				}
				keys.add(option.getKey());

				if (values.contains(option.getValue())) {
					throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
							String.format("Options values %s must be unique", option.getValue()));
				}
		...
		if ((QuestionType.MCQ.value().equalsIgnoreCase(question.getType())
			...
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Answer Options not supplied for question type %s", question.getType()));
		}

		/* tita question's answer */
		if (QuestionType.TITA.value().equalsIgnoreCase(question.getType())
				&& (StringUtils.isEmpty(question.getAnswer().getAnswerText()))) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Answer text not supplied for question type %s", question.getType()));
		}
	}
	...
	public String createQuestion(Question question) {
		QuestionId id = new QuestionId();
		id.setQuestionId(
				String.valueOf(StringUtility.zeroAllocationHash(question.getName() + question.getPassageContent())));
		id.setTenantId(AuthUtils.getCurrentTenantId());
		question.setId(id);
		...
		validateQuestion(question);
		if (exists(id.getQuestionId()))
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Question already exist");
		question.setCreatedBy(AuthUtils.getCurrentQualifiedUsername());
		question.setCreatedOn(new Date());
	...
	@Override
	public QuestionPaginatedResponse searchQuestions(SearchQuestion searchFilter) {
		if (searchFilter.getPageNumber() <= 0) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Invalid page number - %s", searchFilter.getPageNumber()));
		}

		List<String> exclusionQuestionId = new ArrayList<>();
		if (StringUtils.isNotBlank(searchFilter.getTestIdToBeExcluded())) {
			QuestionPaperId id = new QuestionPaperId();
			id.setTenantId(AuthUtils.getCurrentTenantId());
			id.setQuestionPaperId(searchFilter.getTestIdToBeExcluded());
			...
			if (questionPaperToExclude != null) {
				for (PaperSection section : questionPaperToExclude.getSections().values()) {
					if (section != null && section.getQuestions() != null && section.getQuestions().size() > 0)
						section.getQuestions().values().forEach(e -> exclusionQuestionId.add(e.getId()));
				}
			}
		}
		...
		if (StringUtils.isNotEmpty(searchFilter.getQuestionId())) {
			criterias.add(Criteria.where("_id.questionId").is(searchFilter.getQuestionId()));
		}

		if (!exclusionQuestionId.isEmpty()) {
			criterias.add(Criteria.where("_id.questionId").nin(exclusionQuestionId));
		}
		if (StringUtils.isNotEmpty(searchFilter.getType())) {
			criterias.add(Criteria.where("type").is(searchFilter.getType()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getSubject())) {
			criterias.add(Criteria.where("subject").is(searchFilter.getSubject()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getFilename())) {
			criterias.add(Criteria.where("fileName").is(searchFilter.getFilename()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getTopic())) {
			criterias.add(Criteria.where("topic").is(searchFilter.getTopic()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getSubTopic())) {
			criterias.add(Criteria.where("subTopic").is(searchFilter.getSubTopic()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getMigrationTestId())) {
			criterias.add(Criteria.where("migration.test_id").is(searchFilter.getMigrationTestId()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getMigrationTestName())) {
			criterias.add(Criteria.where("migration.test_name").is(searchFilter.getMigrationTestName()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getMigrationSectionId())) {
			criterias.add(Criteria.where("migration.section_id").is(searchFilter.getMigrationSectionId()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getMigrationSectionName())) {
			criterias.add(Criteria.where("migration.section_name").is(searchFilter.getMigrationSectionName()));
		}
		if (searchFilter.getTags() != null && !searchFilter.getTags().isEmpty()) {
		...
		if (StringUtils.isNotEmpty(searchFilter.getUpdateStartTime())) {
			rangeCriteria = Criteria.where("lastUpdatedOn").gte(TimeUtility.convertTxtToDateTime(
					searchFilter.getUpdateStartTime(), TimeUtility.WIDE_FORMAT, StringUtility.getClientTimezone()));
		}
		if (StringUtils.isNotEmpty(searchFilter.getUpdateEndTime())) {
			if (rangeCriteria != null) {
				rangeCriteria.lte(TimeUtility.convertTxtToDateTime(searchFilter.getUpdateEndTime(),
						TimeUtility.WIDE_FORMAT, StringUtility.getClientTimezone()));
				criterias.add(rangeCriteria);
			} else {
				Criteria etc = Criteria.where("lastUpdatedOn").lte(TimeUtility.convertTxtToDateTime(
						searchFilter.getUpdateEndTime(), TimeUtility.WIDE_FORMAT, StringUtility.getClientTimezone()));
				criterias.add(etc);
			}
		}
		...
		if (StringUtils.isNotEmpty(searchFilter.getNameRegexPattern())) {
			Criteria description = Criteria.where("description")
					.regex(Pattern.quote(searchFilter.getNameRegexPattern()), "idx");
			Criteria filename = Criteria.where("fileName").regex(Pattern.quote(searchFilter.getNameRegexPattern()),
					"idx");
			Criteria name = Criteria.where("name").regex(Pattern.quote(searchFilter.getNameRegexPattern()), "idx");
		...
		/* sort */
		if (StringUtils.isNotEmpty(searchFilter.getSortColumn())
				&& Arrays.asList("asc", "desc").contains(String.valueOf(searchFilter.getSortOrder()).toLowerCase())) {
			if ("asc".equalsIgnoreCase(searchFilter.getSortOrder())) {
				query.with(Sort.by(Sort.Direction.ASC, searchFilter.getSortColumn()));
			} else if ("desc".equalsIgnoreCase(searchFilter.getSortOrder())) {
				query.with(Sort.by(Sort.Direction.DESC, searchFilter.getSortColumn()));
			}
		}
		if (!"questionId".equalsIgnoreCase(searchFilter.getSortColumn())) {
			query.with(Sort.by(Sort.Direction.ASC, "_id.questionId"));
		}
		int limit = searchFilter.getPageSize() > 0 && searchFilter.getPageSize() <= 100 ? searchFilter.getPageSize()
				: 100;

		/* get page specific records */
		int actualPage = searchFilter.getPageNumber() > 0 ? searchFilter.getPageNumber() : 1;
		long skipRecords = 0;
		if (actualPage > 1) {
			if (!searchFilter.isNext()) {
				actualPage--;
			}
		...
		if (totalRecords > 0) {
			questions = updateReferences(mongoTemplate.find(query, Question.class), searchFilter);
	...
	public void deleteQuestion(String questionId) {
		...
		if (question.getUsedInPapers() != null && !question.getUsedInPapers().isEmpty()) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					String.format("Can't delete question with id %s. Question is part of %s question papers",
							questionId, question.getUsedInPapers().size()));
		}
	...
	public void updateQuestionTags(String questionId, List<String> tags) {
		...
		id.setTenantId(AuthUtils.getCurrentTenantId());
		Question question = questionRepository.findById(id)
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Question does not exist"));
		...
		while (decoder.hasNext()) {
			...
			List<Question> mappedQuestion = questionsForFile.stream().filter(q -> {
				if ((matchedPassage == null
						...
						&& (StringUtils.isEmpty(questionName) || questionName.replaceAll(" ", "").equals(StringUtility
	...
	@Override
	public QuestionResponseDto getQuestionById(QuestionId id) {
		Question question = questionRepository.findById(id)
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
						String.format("Question with id %s not found", id.getQuestionId())));
		/* create response DTO */
		QuestionResponseDto questionResponseDto = question.getResponseDto();
		/* update passage content */
		updateQuestionPassageReference(questionResponseDto);
		/* update used test details */
		updateUsedInTestReference(true, questionResponseDto);
